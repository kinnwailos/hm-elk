...
Инструкции для скриншотов (что приложить):
- `curl -X GET 'localhost:9200/_cluster/health?pretty'` — показать `cluster_name`.
- Окно Kibana Dev Tools с запросом `GET /_cluster/health?pretty`.
- Discover в Kibana с результатами по `nginx-logs-*` и `filebeat-*` (видны логи).

Файлы конфигурации находятся в репозитории и перечислены выше; для сдачи домашнего задания сделайте `git commit` и `git push` в ваш форк и приложите ссылку в личном кабинете.
